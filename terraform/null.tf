resource "null_resource" "out" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      ../scripts/env-file.sh ${aws_db_instance.rds.address} ${aws_db_instance.rds.port} ${aws_elasticache_cluster.ec-redis.address} ${aws_elasticache_cluster.ec-redis.port}
    EOF
  }
