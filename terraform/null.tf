resource "null_resource" "out" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      ../scripts/env-file.sh ${aws_db_instance.rds.endpoint} ${aws_db_instance.rds.port} ${aws_elasticache_cluster.ec-redis.endpoint} ${aws_elasticache_cluster.ec-redis.port}
    EOF
  }
